...
# 1. Load Data
def load_data(filename='15 scaled.xlsx', sheet_name='Sheet1'):
    try:
...
def get_battery_params():
    ...
    battery_soc_min = 0.1
    battery_soc_init = 0
    battery_charge_eff = 0.7746
    battery_discharge_eff = 0.7746
...
def run_simulation(net, demand_ts, pv_ts, wind_ts, battery_capacity_MWh,  
    ...
    for t in range(len(demand_ts)):
        ...
        max_charge_power_MW = 13085.28
        max_discharge_power_MW = 13085.28
...
def analyze_and_plot(results_df, battery_capacity_MWh):  # Capacity is now expected in MWh
    ...
    print(f"Total fosil fuel Energy: {total_generator_import_energy:.2f} MWh")
...
#----------------------------------------------------------------------------------------------------
    fixed_battery_capacity_MWh = 122445.52 
