...
# 1. Load Data
def load_data(filename='14 scaled - Copy.xlsx', sheet_name='Sheet1'): #excel data name and sheet name modify as needed
    try:
        df = pd.read_excel(filename, sheet_name=sheet_name)
        data_columns = ['Electricity Consumption (MW)', 'PV Generation (MW)', 'Wind Generation (MW)']
        df_data = df[data_columns]
        # convert 15-minute interval data into 1-hour interval data 
        ...
        if 'Timestamp' in df.columns and len(df) > 0:
            ...
            start_datetime = pd.to_datetime('2024-01-01 00:00') 
...
def run_simulation(net, demand_ts, pv_ts, wind_ts, battery_capacity_MWh, battery_power_MW,
    ...
    for t in range(len(demand_ts)):
        ...
        #Use battery_power_MW directly for max charge/discharge
        ...
        try:
            ...
            if generator_power < 0:
                ...
                generator_power = 0 # Ensure generator import is forced to 0
        except Exception as e:
            if not is_pso_run: # Only power flow error
        ...
        results.append({
            'time': t * 60, # Time in minutes for plotting
...
def analyze_and_plot(results_df, battery_capacity_MWh, battery_power_MW):
    ...
    ax1.set_xlabel('Time (minutes)') # hours * 60
...
class BatteryOptimizer:
    def __init__(self, net, demand_ts, pv_ts, wind_ts, battery_params):
        ...
        self.min_capacity = 30000.0  # MWh
        self.max_capacity = 120000 # MWh
        self.min_power_MW = 5000.0  # MW
        self.max_power_MW = 20000.0 # MW
        # Cycle constraints
        self.min_cycles_per_day = 0.22
        self.max_cycles_per_day = 0.23
        self.cycle_penalty = 5.5e7
    ...
    # 2 element array for [capacity, power]
    ...
    def _fitness_function(self, battery_capacity_MWh, battery_power_MW):
        ...
        # 3. Penalty for deviation from target 4-hour duration
    ...
    def optimize(self, n_particles, max_iter):
        ...
        print(f"Cycle penalty: {self.cycle_penalty:.0e}")
...
if __name__ == "__main__":
    ...
    optimal_capacity, optimal_power = optimizer.optimize(n_particles=10, max_iter=2)
#-------------------------------------------------------------------------------
