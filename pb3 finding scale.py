...
solar_to_wind_ratio = ((1 - 0.85) * mean_wind) / (0.85 * mean_pv)
...
hourly_consumption = df['Electricity Consumption (MW)'].sum() #.resample('H')
hourly_wind = df['Wind Generation (MW)'].sum()
hourly_pv = df['PV Generation (MW)'].sum()

# Calculate total energy in MWh for each hour
total_load_energy_mwh = hourly_consumption.sum() / 4 
